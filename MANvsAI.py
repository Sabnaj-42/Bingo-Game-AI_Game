...
def main_menu(RANGE):
    ...
    global PLAY_HOVER, PLAY, GRID_HOVER, N_BLOCKS, grid_input_text, input_active
    ...
    while menu:
        ...
        grid_rect.y = int(HEIGHT / 7 + play_rect.height /7)
        ...
        current_grid_text = newww.render("* Choose among 3-9 *", 1, MENU_CLR)
        ...
        made_by_text = newww.render("* Complete Row/Column/Diagonal>>Score+1 *", 1, MENU_CLR)
        play_line_text = newww.render("* 3*2 or 2*3 Grid>>Score+5 *", 1, MENU_CLR)
        WIN.blit(made_by_text, (int(WIDTH / 2 - made_by_text.get_width() / 2), HEIGHT - 140))
        WIN.blit(play_line_text, (int(WIDTH / 2 - play_line_text.get_width() / 2), HEIGHT - 165))
            ...
            if input_active and event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    try:
                        ...
                        print(grid_input_text)
                        N_BLOCKS = int(grid_input_text)
                        RANGE = N_BLOCKS * N_BLOCKS
                        #grid_input_text = ""
                        #current_grid_text = GRID_FONT.render("Current Grid: {}".format(N_BLOCKS), 1, MENU_CLR)
                        #WIN.blit(current_grid_text, (current_grid_rect.x, current_grid_rect.y))
                        print(N_BLOCKS)
                        pygame.display.flip()
...
def get_best_move(ai_grid, moves):
    ...
    for move in moves:
        ...
        score = minimax(new_grid, depth=6, alpha=float('-inf'), beta=float('inf'), maximizing_player=False,move = move,roww=roww,coll=coll)
       
        #print(" move: ",str(move)," score: ",score)
...
def AI_move(player_grid, grid2,ai_grid,clicked,diag_pos):
    # Find the available numbers that haven't been clicked
        ...
        for row in range(N_BLOCKS):
            for col in range(N_BLOCKS):
                if grid2[row, col] == 0:
                #available.append(player_grid[row, col])
                    moves.append(player_grid[row, col])
                

        #print(moves)

    # If there are available numbers, randomly select one
    
        ai_number = get_best_move(ai_grid,moves) #ekhane ai er move er jonno number select korse
        print("AI selected number:", ai_number)  # Print the number selected by the AI
        for row in range(N_BLOCKS):
            for col in range(N_BLOCKS):
                if player_grid[row, col] == ai_number:
                    # Update the AI's grid and grid2 with the AI's move
                    grid2[row, col] = 1
                    ...
                    #player_grid[row, col] = ai_number #emu
                    for row in range(N_BLOCKS):
                        for col in range(N_BLOCKS):
        ...
        # 3x2 GRIDS
                ...
                # Check for a 3x2 grid
                    ...
                    #show_numbers(player_grid, grid2)
                    return grid2,ai_score,ai_number

    # If all numbers have been clicked, the AI can't make a move
        return
...
def main(RANGE):
    ...
    while run:
        ...
        if not win and human_turn == 0:
            print("AI's move now:")
            grid2,ai_score,ai_number = AI_move(player_grid, grid2, ai_grid,clicked,diag_pos)
            #print("Player Grid")
            #print(player_grid)
